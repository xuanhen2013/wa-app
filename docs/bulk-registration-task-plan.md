...
- `WA_BULK_REGISTRATION_MAX_ITEMS` 和 `WA_BULK_REGISTRATION_CONCURRENCY` 是成本和风控边界，保留为环境变量。
...
默认并发建议为 `1`。批量注册涉及 WA 风控、短信平台扣费和代理 sticky session，先求稳定。
...
- HeroSMS WhatsApp service code 和菲律宾 country id 由 provider adapter 自动发现并缓存，不作为默认环境变量暴露。
...
- 默认目标数量为 `10`，并受 `WA_BULK_REGISTRATION_MAX_ITEMS` 限制。
