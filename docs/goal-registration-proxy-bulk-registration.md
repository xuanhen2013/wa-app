...
### Goal 2 批量提交操作栏修正（2026-07-10）

- 初版提交按钮位于带 `overflow-hidden` 的报价卡片内，导致其不能以页面实际滚动区作为 sticky 参照。
- 已将操作栏移至报价卡片外，作为批量添加页面主滚动区的 `bottom: 0` sticky 操作栏。
- 已发布提交 `2b49d5e` 到独立容器 `wa-app-whats`，运行镜像为 `wa-app:whats-2b49d5e`；宿主机和公网健康检查均通过，旧 `wa-app` 服务未变更。

### Goal 2 报价刷新选择清理（2026-07-11）

- 点击刷新报价时会先清空全部已选数量，再加载新的价格档，避免已下架报价残留在任务选择中。
- 已发布提交 `b4b210c` 到独立容器 `wa-app-whats`，运行镜像为 `wa-app:whats-b4b210c`；宿主机和公网健康检查均通过，未创建任务、未购号、未请求 OTP。
