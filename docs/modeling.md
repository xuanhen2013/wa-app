...
号码探测路径每次生成随机设备指纹但不持久化；配置 `WA_APP_PROXY_RUNTIME_API_BASE_URL` 时通过 proxy-runtime 固定网关 IN-USER 规则选择出口，未配置或不可用时直连降级。注册 OTP 路径使用独立固定网关用户名，但代理始终是可选增强。不做出口 IP、风控、CF 或目标连通性预检，不按 workspace、号码、账号、号码国家或地区直接持有上游代理 lease。

PG/Redis/proxy-runtime 都不是 wa-app 启动必需依赖；PG/Redis 缺省使用 `WA_APP_DATA_DIR` 下的 SQLite durable/runtime，proxy-runtime 缺省直连，外部自动化仅能作为调用方存在。
